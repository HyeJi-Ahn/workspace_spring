...
public class HomeController {
	...
	public String ezen(Model model) {
		// Model 객체는 Request 영역에서 데이터를 저장할 수 있는 객체
	...
	// 백단에서 화면단으로 데이터 전송
	// model: 키는 String, 값은 Object로 되어있어서 어떠한 형태의 값이든 전송가능
	@GetMapping("/hello.do")
...
//		model.addAttribute("list", mapList);
		...
		//    로그인한 유저의 정보를 담아주거나 현재 연결되어 있는 세션의 정보를 담아줄 때 사용
	...
	public String getNameList(Model model) {
		...
		// FIRST_NAME -> firstName(맵으로 사용할 경우 카멜케이스로 변경됨)
...
//	}
	...
	public String updateName(@RequestParam Map<String, Object> paramMap) {
		...
		// redirect:해당 주소 => jsp를 찾아가지 않고 해당 주소를 다시 호출
		// redirect vs forward
